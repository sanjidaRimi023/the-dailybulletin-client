      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between gap-4 items-center">
        ...
        <div className="flex gap-4">
          <select
            ...
            className="px-6 border rounded dark:bg-gray-800 dark:text-white"
            value={sortField}
          >
...
{selectedArticle && !rejectionModalOpen && (
  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
    <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white p-6 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto scrollbar-hide">
      <h3 className="text-2xl font-bold mb-4 text-center">{selectedArticle.title}</h3>
      ...
      <div className="grid gap-2 text-sm sm:text-base">
        ...
        <p><strong>Status:</strong> <span className="capitalize text-yellow-500">{selectedArticle.status}</span></p>
        <p><strong>Premium:</strong> {selectedArticle.isPremium ? "Yes" : "No"}</p>
        <p><strong>Views:</strong> {selectedArticle.viewCount}</p>
        <p><strong>Created At:</strong> {new Date(selectedArticle.createdAt).toLocaleString()}</p>
      </div>
...
{rejectionModalOpen && selectedArticle && (
  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
    <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white p-6 rounded-2xl shadow-2xl w-full max-w-md">
      <h3 className="text-xl font-bold mb-4 text-red-600">Reject Article</h3>
