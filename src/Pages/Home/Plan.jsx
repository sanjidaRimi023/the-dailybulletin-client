      ...
      <div className="container mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-lg">
            Choose a plan that fits your curiosity. Get more with a yearly
            subscription.
          </p>
        </div>
        ...
        <div className="flex justify-center items-center mb-12">
          <span
            className={`mr-3 font-medium ${
              billingCycle === "monthly" ? "text-indigo-400" : "text-gray-500"
            }`}
          >
            Monthly
          </span>
          <button
            ...
            className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${
              billingCycle === "yearly" ? "bg-indigo-700" : "bg-gray-300"
            }`}
          >
            <span
              aria-hidden="true"
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl p-8 shadow-lg transition-transform duration-300 ${
                plan.highlighted ? "bg-white/10 hover:scale-105" : "bg-white/20"
              }`}
            >
              {plan.highlighted && (
              ...
              <Link to="/dashboard/user/subscription" className="mt-10 block">
                <button
                  className={`w-full py-2 px-4 text-base font-semibold rounded-full transition-colors duration-300 ${
                    plan.highlighted
                      ? "bg-indigo-600 text-white hover:bg-indigo-700"
                      : "bg-white text-indigo-600 border border-indigo-300 hover:bg-indigo-300"
                  }`}
                >
                  Choose Plan
                </button>
